...
soslab::GL3::GL3()
{
	std::cout << "GL3 created." << std::endl;
	frameData.rows = 1;
	frameData.cols = 1000;
	frameData.depth[0].resize(1000);
	frameData.intensity[0].resize(1000);
	frameData.points[0].resize(1000);
	angle.resize(1000);
}

soslab::GL3::~GL3()
{
	std::cout << "GL3 deleted." << std::endl;
}

bool soslab::GL3::supports(Feature f) const
{
	switch (f)
	{
	...
	case Feature::FWVersion:
		return true;
	default:
		std::cerr << "Selected feature is not supported in GL3.\n";
		return false;
	}
}

bool soslab::GL3::buildCommand(const Request& req, soslab::MessageBase& msg, std::vector<std::vector<uint8_t>>& totalProtocol)
{
	...
	{
		std::cerr << "Wrong feature to build GL3 command.\n";
		return false;
	}
	...
	{
		// cat
	case Feature::Console:											header.cat = static_cast<uint16_t>(soslab::GL3::Command::Console); break;
	case Feature::OperationMode:									header.cat = static_cast<uint16_t>(soslab::GL3::Command::OperationMode); dtnBytes = BoolMsg2Bytes(msg); break;
	case Feature::StreamEnable:										header.cat = static_cast<uint16_t>(soslab::GL3::Command::StreamEnable); dtnBytes = BoolMsg2Bytes(msg); break;
	case Feature::AreaLevelData:									header.cat = static_cast<uint16_t>(soslab::GL3::Command::AreaLevelData); dtnBytes = AreaInfo2Bytes(msg); break;
	case Feature::AreaDataFinish:									header.cat = static_cast<uint16_t>(soslab::GL3::Command::AreaDataFinish); dtnBytes = BoolMsg2Bytes(msg); break;
	case Feature::SerialNum:										header.cat = static_cast<uint16_t>(soslab::GL3::Command::SerialNum); dtnBytes = StringMsg2Bytes(msg); break;
	case Feature::EthernetInfo:										header.cat = static_cast<uint16_t>(soslab::GL3::Command::EthernetInfo); dtnBytes = req.mode == SetMode::Set ? EthernetInfo2Bytes(msg) : BoolMsg2Bytes(msg); break;
	case Feature::FWVersion:										header.cat = static_cast<uint16_t>(soslab::GL3::Command::FWVersion); dtnBytes = BoolMsg2Bytes(msg); break;
	default:
		std::cerr << " [ERROR] Feature is not supported in GL3 buildCommand.\n";
		return false;
	}

	pageLength = static_cast<uint8_t>(ceil(static_cast<double>(dtnBytes.size()) / static_cast<double>(commandDtnMax)));

	for (uint8_t currentPage = 0; currentPage < pageLength; currentPage++)
	{
		protocol.clear();
		uint8_t cs = 0x00;
...
bool soslab::GL3::parseCommand(const Request& req, const std::vector<uint8_t>& pkt, soslab::MessageBase& out, bool& isFinish)
{
	bool retval = true;
...
soslab::packetStatus soslab::GL3::classifyPacket(const std::vector<uint8_t>& pkt) const
{
...
bool soslab::GL3::parseCommandData(const Request& req, const std::vector<uint8_t>& dtn, soslab::MessageBase& out)
{
	bool retval = true;
	Feature feature = req.feature;
...
bool soslab::GL3::parseAreaInfoAck(const std::vector<uint8_t>& ack, soslab::MessageBase& out)
{
...
bool soslab::GL3::parseEthernetInfoAck(const std::vector<uint8_t>& ack, soslab::MessageBase& out)
{
...
bool soslab::GL3::parseBoolAck(const std::vector<uint8_t>& ack, soslab::MessageBase& out)
{
...
bool soslab::GL3::parseUInt8Ack(const std::vector<uint8_t>& ack, soslab::MessageBase& out)
{
...
bool soslab::GL3::parseUInt16Ack(const std::vector<uint8_t>& ack, soslab::MessageBase& out)
{
...
bool soslab::GL3::parseUInt32Ack(const std::vector<uint8_t>& ack, soslab::MessageBase& out)
{
...
bool soslab::GL3::parseStringAck(const std::vector<uint8_t>& ack, soslab::MessageBase& out)
{
...
bool soslab::GL3::parseDoubleAck(const std::vector<uint8_t>& ack, soslab::MessageBase& out)
{
...
bool soslab::GL3::parseFloatAck(const std::vector<uint8_t>& ack, soslab::MessageBase& out)
{
...
std::vector<uint8_t> soslab::GL3::BoolMsg2Bytes(const soslab::MessageBase& msg)
{
...
std::vector<uint8_t> soslab::GL3::UInt8Msg2Bytes(const soslab::MessageBase& msg)
{
	std::vector<uint8_t> bytes;
...
std::vector<uint8_t> soslab::GL3::UInt16Msg2Bytes(const soslab::MessageBase& msg)
{
	std::vector<uint8_t> bytes;
...
std::vector<uint8_t> soslab::GL3::UInt32Msg2Bytes(const soslab::MessageBase& msg)
{
	std::vector<uint8_t> bytes;
...
std::vector<uint8_t> soslab::GL3::StringMsg2Bytes(const soslab::MessageBase& msg)
{
	std::vector<uint8_t> bytes;
...
std::vector<uint8_t> soslab::GL3::DoubleMsg2Bytes(const soslab::MessageBase& msg)
{
	std::vector<uint8_t> bytes;
...
std::vector<uint8_t> soslab::GL3::FloatMsg2Bytes(const soslab::MessageBase& msg)
{
	std::vector<uint8_t> bytes;
...
std::vector<uint8_t> soslab::GL3::EthernetInfo2Bytes(const soslab::MessageBase& msg)
{
	std::vector<uint8_t> bytes;
...
std::vector<uint8_t> soslab::GL3::AreaInfo2Bytes(const soslab::MessageBase& msg)
{
	std::vector<uint8_t> bytes;
	...
	for (size_t regionNum = 0; regionNum < areaDataMsg->area.size(); regionNum++)
	{
		...
		// coordinates (x, y pairs as int32_t in millimeters)
...
void soslab::GL3::calOutLinePolygon(int frameDataSize, double h_fov, std::vector<double>& coordX, std::vector<double>& coordY, std::vector<double>& outlines)
{
	outlines.clear();
...
void soslab::GL3::calOutLineArc(int frameDataSize, double h_fov, std::vector<double>& coordX, std::vector<double>& coordY, std::vector<double>& outlines)
{
	outlines.clear();
...
void soslab::GL3::convertCoordinate(const std::vector<double>& coordX, const std::vector<double>& coordY, std::vector<double>& distances, std::vector<double>& angles)
{
...
std::vector<double> soslab::GL3::calcCrossingPointWithPolygon(double x0, double y0, double x1, double y1, double ang, double yi)
{
...
bool soslab::GL3::onLine(double x0, double y0, double x1, double y1, double x, double y)
{
...
soslab::Feature soslab::GL3::catToFeature(uint16_t cat) const
{
	switch (cat)
	{
